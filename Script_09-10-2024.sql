...
select pedidos.id as pedido, data, valor, nome from pedidos right join clientes on pedidos.cliente_id = clientes.id; -- Estamos considerando a tabela cliente e não a pedidos.
...
right join clientes on clientes.id = pedidos.cliente_id group by clientes.id, clientes.nome;
...
select count(pedidos.id) as numero_pedidos, clientes.nome from pedidos right join clientes on pedidos.cliente_id = clientes.id group by clientes.nome;
select * from pedidos;

-- 14. Qual o pedido mais caro, exibir o nome do cliente e o valor do pedido
select nome, max(pedidos.valor) as valor_pedido from clientes inner join pedidos on pedidos.cliente_id = clientes.id;
select * from pedidos;
...
select clientes.nome, max(valor), min(valor) from clientes inner join pedidos on pedidos.cliente_id = clientes.id where clientes.nome = 'Laura Madureira';
select * from pedidos;
...
-- 18. Quantas pizzas cada cliente comprou no total?
 ...
 inner join pedidos on pedidos.cliente_id = clientes.id group by cliente_id;
 select * from pedidos;
 select * from itens_pedido;
